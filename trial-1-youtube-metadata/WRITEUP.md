...
PowerShell `curl` sent a broken body (`Request body must be JSON`); a Python `urllib` POST worked. Next.js could not bind port 3000 (already in use) and fell back to 3001. Local yt-dlp returned metadata on the first try, including `/shorts/` URLs. On the Dokploy VPS, YouTube blocked the datacenter IP (`Sign in to confirm you’re not a bot`) for every video, not just Shorts. The API now prefers YouTube Data API v3 when `YOUTUBE_API_KEY` is set, then yt-dlp, then InnerTube.
