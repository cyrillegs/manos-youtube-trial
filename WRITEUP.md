...
PowerShell `curl` sent a broken body (`Request body must be JSON`); a Python `urllib` POST worked. Next.js could not bind port 3000 (already in use) and fell back to 3001. yt-dlp itself returned metadata on the first try for `jNQXAC9IVRw`. With more time I would add a health check that confirms `PYTHON_BIN` and `yt-dlp` at boot, and structured logs around the subprocess (exit code, duration, stderr).
